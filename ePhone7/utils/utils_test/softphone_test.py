from ePhone7.utils.get_softphone import get_softphone, delete_softphone
...
for index in range(len(users) / 2):
    caller_name = users[index * 2]
    called_name = users[index * 2 + 1]
    ...
    delete_softphone(caller)
    delete_softphone(called)
