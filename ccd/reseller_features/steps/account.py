...
def step_impl(context):
    domain_time_frames_view.click_element_by_name('AddTimeFrame')


@then("an Add a Timeframe modal dialog appears")
def step_impl(context):
    domain_time_frames_view.find_element("TimeframeAddForm")


@when("I enter a timeframe name")
def step_impl(context):
    sleep(2)
    domain_time_frames_view.find_element("TimeframeName").send_keys('SVAuto')


@step("I click the Save button")
def step_impl(context):
    sleep(2)
    domain_time_frames_view.click_element_by_name('SaveButton')
