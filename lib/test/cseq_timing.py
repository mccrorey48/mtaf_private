...
for line in lines:
    try:
        (time, src, dst, sip_method, status_code, cseq, cseq_method) = line.split()
        if cseq_method == 'REGISTER':
            continue
    except:
        print len(line.split())
#     if start:
#         secs = float(time) - float(start)
#     elif cseq_method == 'INVITE':
#         start = time
#         secs = 0.0
#     else:
#         continue
#     print fmt % (float(time), secs, src, dst, cseq_method, cseq, status_code, sip_method)
#     cseq_desc = '%s/cseq=%s' % (cseq_method, cseq)
#     if cseq not in cseqs:
#         cseqs.insert(0, cseq)
#     if cseq not in cseq_descs:
#         cseq_descs[cseq] = [cseq_desc]
#     elif cseq_desc in cseq_descs[cseq]:
#         # throw away repeats
#         continue
#     else:
#         cseq_descs[cseq].append(cseq_desc)
#     ms = int(secs * 1000)
#     d = {'ms': ms, 'secs': secs, 'desc': cseq_desc, 'src': src, 'dst': dst, 'cseq_method': cseq_method, 'sip_method': sip_method[:3]}
#     if cseq in cseq_ds:
...
#             y_offsets[d['ms']] = inv_offsets[sip_method]
#         elif cseq_method[:3] == 'BYE':
#             y_offsets[d['ms']] = bye_offsets[sip_method]
